...
> **📦 本仓库说明（再发布 / 定制版）**
>
> 本仓库是 [dsh-vision-router](https://github.com/ysr666/dsh-vision-router)（**MIT 协议，原作者 [ysr666](https://github.com/ysr666) 原创**）的再发布 / 定制版，由 [亿哲学长](https://github.com/BillionSeniors) 维护。
>
> **图片识别是哪个？** 本仓库的核心识图能力 —— `vision_describe` / `vision_ocr` / `vision_ground` / `vision_crop` 等全套像素级视觉工具、内置免费 OVH 兜底链、图片轮自动路由 —— **全部为原作者 ysr666 原创**。识图时具体由哪个 AI「看图」由用户配置的视觉后端决定（默认可配智谱 / 豆包 / Gemini / OpenAI 等任意 OpenAI 兼容视觉模型），插件负责把图片交给它识别并把结果带回给 DeepSeek 推理。
>
...
>
> **🧩 模型页企业级编辑器（核心补丁，v1.3.4+ 全自动）**：插件把「设置 → 模型 → 视觉路由 → 编辑」升级为分组卡片式企业界面（直连提供方 / 服务商预设 / API Key 直填）。**插件一加载就会自动检测并应用补丁**——装完即生效，无需任何手动命令；`dsh plugin add` 之后重启一次 DSH（或刷新页面）即可看到新界面。
>
> v1.3.4 起，编辑器**保存时自动补全 HTTP 提供方行的缺失字段**（名称 / API 地址 / 模型从底层值补齐）——即使只粘贴了 Key 也不会再保存出残缺记录；历史旧补丁会自动升级。自动应用是幂等且安全的：只处理与发布基线逐字节一致（0.1.0-rc.6）的核心，打前自动备份原文件（`client.js.orig-bak`），版本不匹配自动跳过、绝不破坏安装；补丁已存在则直接跳过。DSH 升级覆盖核心后，插件下次加载会自动重新打上。
>
> 仍可手动执行（查看状态 / 补打）：
>
> ```sh
