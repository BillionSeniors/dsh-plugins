...
- **界面怎么显示当前用的谁**：改完保存即生效 —— 聊天页模型选择器显示固定的 `DeepSeek + 自动识图` 组名；设置页路由行显示 `视觉路由（自动识图）· 智谱 glm-4v-flash`；未配置任何 Key 时显示「内置免费链（OVH）」
...
实际使用效果：聊天里上传图片，AI 自动调用 `vision_describe` 等像素级视觉工具识别：

![AI 识别图片实际使用效果](assets/vision-describe-usage.png)
