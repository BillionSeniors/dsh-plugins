...
export function apply(ctx, config = {}) {
  ...
  // ── 当前视觉后端名称（显示在模型选择器 / 设置页）──────────────────
  const friendlyVendor = (baseURL) => {
    const u = String(baseURL || '').toLowerCase()
    if (u.includes('bigmodel.cn')) return '智谱'
    if (u.includes('volces.com')) return '豆包'
    if (u.includes('generativelanguage.googleapis.com')) return 'Gemini'
    if (u.includes('openai.com')) return 'OpenAI'
    if (u.includes('anthropic.com')) return 'Anthropic'
    if (u.includes('moonshot')) return 'Kimi'
    if (u.includes('dashscope') || u.includes('aliyuncs.com')) return '通义'
    if (u.includes('deepseek.com')) return 'DeepSeek'
    if (u.includes('ovh.net')) return 'OVH'
    return ''
  }
  const visionBackendName = () => {
    const cfg = current()
    const hps = (Array.isArray(cfg.httpProviders) ? cfg.httpProviders : []).filter(
      (p) => p && typeof p.baseURL === 'string' && typeof p.model === 'string',
    )
    if (hps.length > 0) {
      const p = hps[0]
      return `${friendlyVendor(p.baseURL)} ${p.model}`.trim()
    }
    const ps = (Array.isArray(cfg.providers) ? cfg.providers : []).filter(
      (x) => x && typeof x.provider === 'string',
    )
    if (ps.length > 0) return ps[0].provider === 'vision-http' ? '内置免费链（OVH）' : `${ps[0].provider}/${ps[0].model}`
    return '内置免费链（OVH）'
  }
  ...
  ctx.inject(['settings'], (sctx) => {
    ...
    current = () => scope.get()
    // Expose the namespace to the web configuration boundary. Registered AFTER
    // settings mount so displayName shows the resolved vision backend (e.g.
    // 智谱 / 豆包 / Gemini), not the pre-settings composition defaults. The API
    // proxy serves settings describe/mutate ONLY for configurable-provider
