window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		// Enterprise-style classes for the generic (schema-driven) provider editor.
		const css$4 = ".genx_section{border:1px solid var(--dsw-alias-border-l2);border-radius:12px;padding:14px 16px;margin:12px 0 0;background:var(--dsw-alias-bg-layer-2,transparent)}.genx_sectionTitle{margin:0 0 2px;font-size:13px;font-weight:600;color:var(--dsw-alias-label-primary);cursor:pointer;user-select:none;display:flex;align-items:center;gap:6px}.genx_sectionTitle::before{content:\"▸\";font-size:11px;color:var(--dsw-alias-label-tertiary);transition:transform .12s ease}.genx_section[open] .genx_sectionTitle::before{transform:rotate(90deg)}.genx_sectionDesc{margin:0 0 10px;font-size:12px;color:var(--dsw-alias-label-tertiary)}.genx_grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(250px,1fr));gap:10px 14px}.genx_field{display:flex;flex-direction:column;gap:4px;min-width:0}.genx_label{font-size:12px;color:var(--dsw-alias-label-secondary);word-break:break-word}.genx_input{width:100%;box-sizing:border-box;min-height:34px;padding:6px 10px;border:1px solid var(--dsw-alias-border-l2);border-radius:8px;background:var(--dsw-alias-bg-layer-1,transparent);color:var(--dsw-alias-label-primary);font:inherit;font-size:13px}.genx_area{min-height:64px;font-family:ui-monospace,SFMono-Regular,Consolas,monospace;font-size:12px;line-height:1.5;resize:vertical}.genx_toggle{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:10px 12px;border:1px solid var(--dsw-alias-border-l2);border-radius:10px;background:var(--dsw-alias-bg-layer-1,transparent)}.genx_toggleText{min-width:0}.genx_switch{position:relative;width:36px;height:20px;flex:none;appearance:none;background:var(--dsw-alias-bg-layer-3,#d0d4e0);border-radius:10px;cursor:pointer;transition:background .15s ease;margin:0}.genx_switch:checked{background:var(--dsw-alias-state-success-primary,#2ea44f)}.genx_switch::after{content:\"\";position:absolute;top:2px;left:2px;width:16px;height:16px;border-radius:50%;background:#fff;box-shadow:0 1px 2px rgba(0,0,0,.25);transition:left .15s ease}.genx_switch:checked::after{left:18px}.genx_providerRow{border:1px solid var(--dsw-alias-border-l2);border-radius:12px;padding:12px 14px;margin-bottom:10px;background:var(--dsw-alias-bg-layer-1,transparent)}.genx_providerHead{display:flex;align-items:center;gap:8px;margin-bottom:10px;flex-wrap:wrap}.genx_rowGrid{display:grid;grid-template-columns:repeat(auto-fill,minmax(210px,1fr));gap:8px 12px}.genx_actions{display:flex;gap:8px;align-items:center;margin-top:8px;flex-wrap:wrap}.genx_hint{font-size:12px;color:var(--dsw-alias-label-tertiary);margin:0}.genx_jsonError{color:var(--dsw-alias-state-error-primary);font-size:12px;margin:4px 0 0}";
		const tagId$4 = "@deepseek-ai/dsh-client-ui-settings-models/GenericEditor.module.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId$4) + "]") === null) {
			const tag2 = document.createElement("style");
			tag2.dataset.plugin = "@deepseek-ai/dsh-client-ui-settings-models";
			tag2.dataset.pluginCss = tagId$4;
			tag2.textContent = css$4;
			document.head.appendChild(tag2);
		}
		const genx = {
			section: "genx_section",
			sectionTitle: "genx_sectionTitle",
			sectionDesc: "genx_sectionDesc",
			grid: "genx_grid",
			field: "genx_field",
			label: "genx_label",
			input: "genx_input",
			area: "genx_area",
			toggle: "genx_toggle",
			toggleText: "genx_toggleText",
			switch: "genx_switch",
			providerRow: "genx_providerRow",
			providerHead: "genx_providerHead",
			rowGrid: "genx_rowGrid",
			actions: "genx_actions",
			hint: "genx_hint",
			jsonError: "genx_jsonError"
		};
		/** Common OpenAI-compatible endpoint presets offered by the generic provider-row editor. */
		const GENERIC_HTTP_PRESETS = [
			{ id: "custom", label: "genericPresetCustom", baseURL: "", model: "" },
			{ id: "openai", label: "OpenAI", baseURL: "https://api.openai.com/v1", model: "gpt-4o-mini" },
			{ id: "anthropic", label: "Anthropic Claude", baseURL: "https://api.anthropic.com/v1", model: "" },
			{ id: "zhipu", label: "智谱 GLM", baseURL: "https://open.bigmodel.cn/api/paas/v4", model: "glm-4v-flash" },
			{ id: "ark", label: "火山方舟（豆包）", baseURL: "https://ark.cn-beijing.volces.com/api/v3", model: "doubao-1.5-vision-pro-32k-250115" },
			{ id: "dashscope", label: "阿里云百炼（通义 VL）", baseURL: "https://dashscope.aliyuncs.com/compatible-mode/v1", model: "qwen-vl-plus" },
			{ id: "siliconflow", label: "SiliconFlow", baseURL: "https://api.siliconflow.cn/v1", model: "Qwen/Qwen2.5-VL-72B-Instruct" },
			{ id: "openrouter", label: "OpenRouter", baseURL: "https://openrouter.ai/api/v1", model: "qwen/qwen3-vl-235b" }
		];
		/** camelCase key -> display label ("httpProviders" -> "HTTP Providers"). */
		function prettyFieldName(name) {
			const parts = String(name).replace(/([a-z0-9])([A-Z])/g, "$1 $2").split(/[\s_\-]+/).filter((p) => p !== "");
			return parts.map((part) => /^(api|url|id|ttl|http|json)$/i.test(part) ? part.toUpperCase() : part.charAt(0).toUpperCase() + part.slice(1)).join(" ");
		}
		...
		*/
		...
		/**
		* For namespaces that keep their keys inside the settings section itself
		* (no root apiKeyEnv reference — e.g. vision-router's httpProviders[].apiKey),
		* detect whether a key is present and whether the user has configured any
		* provider entries. Drives the same configured/missing status dot as the
		* credential-backed rows.
		*/
		function scanDirectKeys(namespace, path) {
			const user = (0, _deepseek_ai_dsh_client_schema_form.getPath)(namespace.user, path);
			const value = (0, _deepseek_ai_dsh_client_schema_form.getPath)(namespace.value, path);
			const keyed = (section) => {
				if (typeof section !== "object" || section === null) return false;
				for (const [key, v] of Object.entries(section)) {
					if (typeof v === "string" && v !== "" && /^(apiKey|api_key|key|token|secret)$/i.test(key)) return true;
					if (Array.isArray(v)) {
						for (const entry of v) {
							if (entry && typeof entry === "object" && typeof entry.apiKey === "string" && entry.apiKey !== "") return true;
						}
					}
				}
				return false;
			...
			const configured = (section) => {
				if (typeof section !== "object" || section === null) return false;
				return Object.values(section).some((v) => Array.isArray(v) && v.length > 0);
			};
			return { keyed: keyed(value), configured: configured(user) };
		}
			...
			*/
			async load() {
				...
				const rows = providers.map((entry) => {
					...
					const envRef = apiKeyEnvOf(namespace, entry.settingsPath);
					const direct = envRef === void 0 && namespace !== void 0 ? scanDirectKeys(namespace, entry.settingsPath) : { keyed: false, configured: false };
					return {
						entry,
		...
		*/
		function ProviderEditor(props) {
			...
			/** Raw JSON text for generic-editor fields the typed renderers cannot handle. */
			const [genericJson, setGenericJson] = (0, react.useState)({});
			...
			/**
			* Unknown namespaces whose section schema is a plain object get a
			* schema-driven generic editor (all fields editable and saved through
			* the normal settings seam) instead of the read-only "edit settings.yaml"
			* hint. Known families and unresolvable nodes keep their current behavior.
			*/
			const editorLayout = layout === "unknown" && props.credentialOnly !== true && node !== void 0 && node.type === "object" ? "settings" : layout;
			const genericJsonInvalid = Object.entries(genericJson).some(([, raw]) => {
			...
			*/
			...
			/**
			* Schema-driven generic editor for namespaces the product does not ship
			* a curated family for (e.g. third-party configurable providers like
			* vision-router). Renders every field of the section's object schema:
			* booleans as checkboxes, numbers as number inputs, strings as text
			* (password for key-like names), string arrays as newline textareas,
			* object arrays as per-row editors, and anything else as a JSON textarea
			* with parse validation. Edits land in the user layer through the same
			* path ops and settings mutate as the curated editors, and the key-like
			* fields persist inside the settings document itself.
			*/
						const genericEditor = () => {
				const gnode = (name) => node.type === "object" && node.dict ? node.dict[name] : void 0;
				const fieldLabel = (name, fnode) => {
					const desc = fnode && fnode.meta && typeof fnode.meta.description === "string" && fnode.meta.description.trim() !== "" ? fnode.meta.description.trim() : void 0;
					return desc !== void 0 ? desc : prettyFieldName(name);
				};
				const gvalue = (name) => {
					const fromDraft = (0, _deepseek_ai_dsh_client_schema_form.getPath)(draft, [name]);
					if (fromDraft !== void 0) return fromDraft;
					const fromFallback = (0, _deepseek_ai_dsh_client_schema_form.getPath)(fallback, [name]);
					if (fromFallback !== void 0) return fromFallback;
					const fromBase = (0, _deepseek_ai_dsh_client_schema_form.getPath)(namespace.base, [...settingsPath, name]);
					if (fromBase !== void 0) return fromBase;
					const fnode = gnode(name);
					if (fnode && fnode.meta && fnode.meta.default !== void 0) return fnode.meta.default;
					return void 0;
				};
				const gset = (name, next) => setDraft((current) => next === void 0 ? (0, _deepseek_ai_dsh_client_schema_form.deletePath)(current, [name]) : (0, _deepseek_ai_dsh_client_schema_form.setPath)(current, [name], next));
				const rowValue = (name, index, field) => {
					const fromDraft = (0, _deepseek_ai_dsh_client_schema_form.getPath)(draft, [name, index, field]);
					if (fromDraft !== void 0) return fromDraft;
					const fromFallback = (0, _deepseek_ai_dsh_client_schema_form.getPath)(fallback, [name, index, field]);
					if (fromFallback !== void 0) return fromFallback;
					return (0, _deepseek_ai_dsh_client_schema_form.getPath)(namespace.base, [...settingsPath, name, index, field]);
				};
				const rowSet = (name, index, field, next) => setDraft((current) => {
					const path = [name, index, field];
					if (next === void 0) return (0, _deepseek_ai_dsh_client_schema_form.deletePath)(current, path);
					return (0, _deepseek_ai_dsh_client_schema_form.setPath)(current, path, next);
				});
				/** Text / number input (password for key-like names). */
				const textInput = (label, value, secret, numeric, onCommit) => (0, react_jsx_runtime.jsxs)("div", {
					className: genx.field,
					children: [(0, react_jsx_runtime.jsx)("span", {
						className: genx.label,
						children: label
					}), (0, react_jsx_runtime.jsx)("input", {
						className: genx.input, type: numeric === true ? "number" : secret ? "password" : "text", autoComplete: "off",
						value: typeof value === "number" ? String(value) : typeof value === "string" ? value : "", disabled,
						placeholder: secret && typeof value === "string" && value !== "" ? t("genericKeySaved") : "",
						onChange: (event) => {
							if (numeric === true) {
								const n = Number(event.target.value);
								onCommit(event.target.value === "" || !Number.isFinite(n) ? void 0 : n);
							} else {
								onCommit(event.target.value === "" ? void 0 : event.target.value);
							}
						}
					})]
				});
				/** Toggle switch (explicit true/false always committed). */
				const toggleInput = (label, value, onCommit) => (0, react_jsx_runtime.jsxs)("label", {
					className: genx.toggle,
					children: [(0, react_jsx_runtime.jsx)("span", {
						className: genx.toggleText + " " + genx.label,
						children: label
					}), (0, react_jsx_runtime.jsx)("input", {
						type: "checkbox", className: genx.switch, checked: value === true, disabled,
						onChange: (event) => onCommit(event.target.checked === true)
					})]
				});
				/** One scalar field by schema type. */
				const scalarField = (name, fnode, value, onCommit) => {
					const label = fieldLabel(name, fnode);
					if (fnode.type === "boolean") return toggleInput(label, value === true, onCommit);
					if (fnode.type === "number") return textInput(label, value, false, true, onCommit);
					const secret = name === "apiKey" || /secret|token/i.test(name);
					return textInput(label, value, secret, false, onCommit);
				};
				/** One row of an object-array field (e.g. an httpProviders entry). */
				const objectRowEditor = (name, element, rows, index) => {
					// apiKeyEnv is legacy: keys are pasted directly now, so the
					// field is hidden from the editor (the schema keeps it for
					// backward compatibility with existing configurations).
					const fields = Object.keys(element.dict).filter((f) => f !== "apiKeyEnv");
					const hasPreset = fields.includes("baseURL") && (fields.includes("name") || fields.includes("model"));
					const presetOf = () => {
						const baseURL = typeof rowValue(name, index, "baseURL") === "string" ? rowValue(name, index, "baseURL") : "";
						const hit = GENERIC_HTTP_PRESETS.find((p) => p.id !== "custom" && p.baseURL !== "" && p.baseURL === baseURL);
						return hit ? hit.id : "custom";
					};
					return (0, react_jsx_runtime.jsxs)("div", {
						className: genx.providerRow,
						children: [hasPreset ? (0, react_jsx_runtime.jsx)("div", {
							className: genx.providerHead,
							children: [(0, react_jsx_runtime.jsx)("span", {
								className: genx.label,
								children: t("genericPreset")
							}), (0, react_jsx_runtime.jsx)("select", {
								className: genx.input, style: { maxWidth: 280 }, value: presetOf(), disabled,
								onChange: (event) => {
									const preset = GENERIC_HTTP_PRESETS.find((p) => p.id === event.target.value);
									if (!preset || preset.id === "custom") return;
									const currentName = typeof rowValue(name, index, "name") === "string" ? rowValue(name, index, "name") : "";
									const nameMatches = GENERIC_HTTP_PRESETS.some((p) => p.id !== "custom" && p.id === currentName);
									if (fields.includes("name") && (currentName === "" || nameMatches)) rowSet(name, index, "name", preset.id);
									if (fields.includes("baseURL")) rowSet(name, index, "baseURL", preset.baseURL);
									const currentModel = typeof rowValue(name, index, "model") === "string" ? rowValue(name, index, "model") : "";
									if (fields.includes("model") && preset.model !== "" && currentModel === "") rowSet(name, index, "model", preset.model);
								},
								children: GENERIC_HTTP_PRESETS.map((preset) => (0, react_jsx_runtime.jsx)("option", {
									value: preset.id,
									children: preset.label === "genericPresetCustom" ? t(preset.label) : preset.label
								}, preset.id))
							})]
						}) : null, (0, react_jsx_runtime.jsx)("div", {
							className: genx.rowGrid,
							children: fields.map((field) => {
								const fnode2 = element.dict[field];
								const rv = rowValue(name, index, field);
								return scalarField(field, fnode2, rv, (next) => rowSet(name, index, field, next));
							})
						}), (0, react_jsx_runtime.jsx)("div", {
							className: genx.actions,
							children: (0, react_jsx_runtime.jsx)("button", {
								type: "button", className: ModelsSection_module_css_default["secondaryButton"], disabled,
								onClick: () => {
									const list = rows.filter((_r, i) => i !== index);
									gset(name, list.length > 0 ? list : void 0);
								},
								children: t("genericArrayRemove")
							})
						})]
					});
				};
				/** Full field renderer (scalar / array / JSON fallback). */
				const renderField = (name, fnode) => {
					const value = gvalue(name);
					if (fnode.type === "boolean" || fnode.type === "number" || fnode.type === "string") {
						return scalarField(name, fnode, value, (next) => gset(name, next));
					}
					if (fnode.type === "array") {
						const element = fnode.inner;
						const rows = Array.isArray(value) ? value : [];
						if (element && element.type === "object" && element.dict) {
							return (0, react_jsx_runtime.jsxs)("div", {
								className: genx.field,
								children: [(0, react_jsx_runtime.jsx)("span", {
									className: genx.label,
									children: fieldLabel(name, fnode)
								}), rows.map((_row, index) => objectRowEditor(name, element, rows, index)), (0, react_jsx_runtime.jsx)("div", {
									className: genx.actions,
									children: (0, react_jsx_runtime.jsx)("button", {
										type: "button", className: ModelsSection_module_css_default["addButton"], disabled,
										onClick: () => {
											const fresh = {};
											for (const [field, fnode2] of Object.entries(element.dict)) {
												if (fnode2.meta && fnode2.meta.default !== void 0) fresh[field] = fnode2.meta.default;
											}
											gset(name, [...rows, fresh]);
										},
										children: t("genericArrayAdd")
									})
								})]
							});
						}
						if (element && element.type === "string") {
							...
							return (0, react_jsx_runtime.jsxs)("div", {
								...
								children: [(0, react_jsx_runtime.jsx)("span", {
									...
									className: genx.input + " " + genx.area, value: text, disabled,
									onChange: (event) => {
										const list = String(event.target.value).split("\n").map((s) => s.trim()).filter((s) => s !== "");
										gset(name, list.length > 0 ? list : void 0);
									}
								})]
							});
					...
					// Nested object: render its own fields inline (e.g. textProvider).
					if (fnode.type === "object" && fnode.dict) {
						const objFields = Object.keys(fnode.dict).filter((f) => f !== "apiKeyEnv");
						const objValue = (field) => {
							const fromDraft = (0, _deepseek_ai_dsh_client_schema_form.getPath)(draft, [name, field]);
							if (fromDraft !== void 0) return fromDraft;
							const fromFallback = (0, _deepseek_ai_dsh_client_schema_form.getPath)(fallback, [name, field]);
							if (fromFallback !== void 0) return fromFallback;
							const fromBase = (0, _deepseek_ai_dsh_client_schema_form.getPath)(namespace.base, [...settingsPath, name, field]);
							if (fromBase !== void 0) return fromBase;
							const fnode2 = fnode.dict[field];
							if (fnode2 && fnode2.meta && fnode2.meta.default !== void 0) return fnode2.meta.default;
							return void 0;
						};
						const objSet = (field, next) => setDraft((current) => {
						...
						return (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [(0, react_jsx_runtime.jsx)("span", {
								...
								children: (0, react_jsx_runtime.jsx)("div", {
									className: genx.rowGrid,
									children: objFields.map((field) => scalarField(field, fnode.dict[field], objValue(field), (next) => objSet(field, next)))
								})
							})]
						});
					}
					// Exotic types: raw JSON with parse validation.
					const raw = genericJson[name] ?? JSON.stringify(value ?? null, null, 2);
					const invalid = genericJson[name] !== void 0 && genericJsonInvalid;
					return (0, react_jsx_runtime.jsxs)("div", {
						className: genx.field,
						children: [(0, react_jsx_runtime.jsx)("span", {
							className: genx.label,
							...
							className: genx.input + " " + genx.area, style: { minHeight: 96 }, value: raw, disabled,
							onChange: (event) => {
								const text = event.target.value;
								setGenericJson((prev) => ({ ...prev, [name]: text }));
								try {
									const parsed = JSON.parse(text);
									gset(name, parsed === null ? void 0 : parsed);
								} catch {
									/* keep the last good draft; the invalid flag disables Save */
								}
							}
						}), invalid ? (0, react_jsx_runtime.jsx)("p", {
							className: genx.jsonError,
							children: t("genericJsonError")
						}) : null]
					});
				};
				const fields = node.type === "object" && node.dict ? Object.keys(node.dict) : [];
				if (fields.length === 0) {
					return (0, react_jsx_runtime.jsx)("p", {
						className: ModelsSection_module_css_default["advancedHint"],
						children: t("genericNoFields")
					});
				}
				// Provider-like object lists (element schema carries a baseURL,
				// e.g. httpProviders) render first, always expanded.
				const isProviderList = (name) => {
					const f = gnode(name);
					if (!f || f.type !== "array" || !f.inner) return false;
					const el = f.inner;
					return el.type === "object" && el.dict && "baseURL" in el.dict;
				};
				const providerLists = fields.filter((name) => isProviderList(name));
				const basics = fields.filter((name) => {
					const f = gnode(name);
					return f && (f.type === "string" || f.type === "number");
				});
				const switches = fields.filter((name) => {
					const f = gnode(name);
					return f && f.type === "boolean";
				});
				const lists = fields.filter((name) => {
					const f = gnode(name);
					return f && (f.type === "array" && !isProviderList(name) || f.type === "object" && f.dict);
				});
				const others = fields.filter((name) => {
					const f = gnode(name);
					return f && f.type !== "string" && f.type !== "number" && f.type !== "boolean" && f.type !== "array" && !(f.type === "object" && f.dict);
				});
				// Collapsed-by-default accordion section.
				const section = (title, children) => (0, react_jsx_runtime.jsxs)("details", {
					className: genx.section,
					children: [(0, react_jsx_runtime.jsx)("summary", {
						className: genx.sectionTitle,
						children: title
					}), (0, react_jsx_runtime.jsx)("div", {
						style: { marginTop: 10 },
						children
				...
				const openSection = (title, children) => (0, react_jsx_runtime.jsxs)("div", {
					className: genx.section,
				...
				return (0, react_jsx_runtime.jsxs)("div", {
					children: [providerLists.length > 0 ? openSection(t("genericSectionProviders"), (0, react_jsx_runtime.jsx)("div", {
						children: providerLists.map((name) => renderField(name, gnode(name)))
					})) : null, basics.length > 0 ? section(t("genericSectionBasics"), (0, react_jsx_runtime.jsx)("div", {
						className: genx.grid,
						children: basics.map((name) => renderField(name, gnode(name)))
					})) : null, switches.length > 0 ? section(t("genericSectionSwitches"), (0, react_jsx_runtime.jsx)("div", {
						className: genx.grid,
						children: switches.map((name) => renderField(name, gnode(name)))
					})) : null, lists.length > 0 ? section(t("genericSectionLists"), (0, react_jsx_runtime.jsx)("div", {
						children: lists.map((name) => renderField(name, gnode(name)))
					})) : null, others.length > 0 ? section(t("genericSectionJson"), (0, react_jsx_runtime.jsx)("div", {
						children: others.map((name) => renderField(name, gnode(name)))
					})) : null]
				});
			};
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					editorLayout === "unknown" ? (0, react_jsx_runtime.jsx)("p", {
						className: ModelsSection_module_css_default["advancedHint"],
						children: `${t("advancedHint")} (${namespace.ns})`
					}) : editorLayout === "settings" ? genericEditor() : curatedFields(layout),
					failure !== void 0 ? (0, react_jsx_runtime.jsx)("p", {
						className: ModelsSection_module_css_default["error"],
						children: failure
					}) : null,
					...
					(0, react_jsx_runtime.jsx)(EditorFooter, {
						t,
						busy,
						submitDisabled: disabled || editorLayout === "unknown" || genericJsonInvalid || props.credentialOnly !== true && modelFailure !== void 0 || shownKeyFailure !== void 0 || props.credentialRequired === true && keyValue.length === 0,
				...
				children: [
					...
					(0, react_jsx_runtime.jsx)("ul", {
						...
						children: configured.map((row) => {
							...
							const credentialConfigured = row.credential?.configured === true || row.directKeyed === true;
							const credentialMissing = !credentialConfigured && (row.apiKeyEnv !== void 0 && row.credential?.configured === false || row.directConfigured === true && row.directKeyed !== true);
		...
		*/
		...
		const en = {
			...
			genericHint: "This provider's fields come from its own plugin configuration. Edit them here and save — no need to touch settings.yaml.",
			genericJsonError: "Invalid JSON. Fix it to enable Save.",
			genericArrayAdd: "+ Add item",
			genericArrayRemove: "Remove",
			genericKeySaved: "Saved — type a new value to replace",
			genericNoFields: "This provider has no editable fields.",
			genericSectionBasics: "Basics",
			genericSectionSwitches: "Switches",
			genericSectionLists: "Lists",
			genericSectionJson: "JSON (advanced)",
			genericSectionProviders: "Direct providers (paste API key)",
			genericPresetCustom: "Custom",
		...
		const zh = {
			...
			genericHint: "该提供方的字段由插件配置自动生成，直接在此编辑并保存即可，无需手动修改 settings.yaml。",
			genericJsonError: "JSON 无效，请修正后保存。",
			genericArrayAdd: "+ 添加一项",
			genericArrayRemove: "移除",
			genericKeySaved: "已保存——输入新值可替换",
			genericNoFields: "该提供方没有可编辑字段。",
			genericSectionBasics: "基础配置",
			genericSectionSwitches: "行为开关",
			genericSectionLists: "列表",
			genericSectionJson: "JSON（高级）",
			genericSectionProviders: "直连提供方（API Key 直填）",
			genericPresetCustom: "自定义",
			genericPreset: "服务商预设",
