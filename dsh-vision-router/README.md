...
> **📦 Repo notice (re-published / customized fork)**
>
> This repository is a re-published, customized copy of [dsh-vision-router](https://github.com/ysr666/dsh-vision-router) — **MIT licensed, originally created by [ysr666](https://github.com/ysr666)** — maintained by [亿哲学长](https://github.com/BillionSeniors).
>
> **Which AI does the image recognition?** All core vision capabilities — `vision_describe` / `vision_ocr` / `vision_ground` / `vision_crop` and the rest of the pixel-level tool family, the built-in free OVH fallback chain and image-turn auto-routing — **are original work by ysr666**. The actual "seeing" model is user-configurable (Zhipu / Doubao / Gemini / OpenAI or any OpenAI-compatible vision backend); the plugin hands the image to that model and feeds the result back to DeepSeek for reasoning.
>
...
>
> 🔒 **Privacy**: this repository contains **no API keys**. Vision backends read their keys from **local environment variables** (e.g. `ZHIPU_API_KEY` / `ARK_API_KEY`); keys are never distributed with the repo.
